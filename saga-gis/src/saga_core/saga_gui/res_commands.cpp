...
wxString CMD_Get_Name(int Cmd_ID)
{
	...
	case ID_CMD_DATA_LEGEND_BACKGROUND_INVERT: return( _TL("Inverse Colors") );
	...
	case ID_CMD_DATA_SAVEAS_IMAGE            : return( _TL("Save as Image...") );
	...
	case ID_CMD_DATA_CLIPBOARD_COPY          : return( _TL("Copy to Clipboard") );
	...
	case ID_CMD_DATA_FORCE_UPDATE            : return( _TL("Force Refresh") );
	case ID_CMD_DATA_SETTINGS_LOAD           : return( _TL("Load Settings") );
	case ID_CMD_DATA_SETTINGS_COPY           : return( _TL("Copy Settings") );
	case ID_CMD_DATA_RANGE_MINMAX            : return( _TL("Adjust Colors to Minimum/Maximum") );
	case ID_CMD_DATA_RANGE_STDDEV            : return( _TL("Adjust Colors to Standard Deviation") );
	case ID_CMD_DATA_RANGE_STDDEV150         : return( _TL("Adjust Colors to Standard Deviation (1.5)") );
	case ID_CMD_DATA_RANGE_STDDEV200         : return( _TL("Adjust Colors to Standard Deviation (2.0)") );
	case ID_CMD_DATA_RANGE_PERCENTILE        : return( _TL("Adjust Colors to Percentile") );
	case ID_CMD_DATA_CLASSIFY                : return( _TL("Classify") );
	case ID_CMD_DATA_CLASSIFY_IMPORT         : return( _TL("Import Classification") );
	case ID_CMD_DATA_SELECTION_CLEAR         : return( _TL("Clear Selection") );
	case ID_CMD_DATA_SELECTION_INVERT        : return( _TL("Invert Selection") );
	case ID_CMD_DATA_SELECTION_DELETE        : return( _TL("Delete Selection") );
	...
	case ID_CMD_DATA_SHOW_MAP                : return( _TL("Show in Map") );
	case ID_CMD_DATA_HISTOGRAM               : return( _TL("Histogram") );
	case ID_CMD_DATA_SCATTERPLOT             : return( _TL("Scatterplot") );
	...
	case ID_CMD_TABLE_SELECTION_SHOW         : return( _TL("Show Selection") );
	...
	case ID_CMD_SHAPES_EDIT_SEL_COPY         : return( _TL("Copy Selection to New Layer") );
	...
	//-----------------------------------------------------
	// Active Object's Properties...

	case ID_CMD_ACTIVE_CLIPBOARD_COPY        : return( _TL("Copy to Clipboard") );


	//-----------------------------------------------------
	// Views...

	case ID_CMD_VIEW_CLIPBOARD_COPY          : return( _TL("Copy to Clipboard") );
	case ID_CMD_VIEW_CLIPBOARD_PASTE         : return( _TL("Paste from Clipboard") );
	case ID_CMD_VIEW_SAVE                    : return( _TL("Save") );
...
int CMD_Get_ImageID(int Cmd_ID)
{
	...
	case ID_CMD_VIEW_CLIPBOARD_COPY          : return( ID_IMG_TB_CLIPBOARD );
	...
	case ID_CMD_LAYOUT_EXPORT                : return( ID_IMG_TB_LAYOUT_EXPORT );
	...
	case ID_CMD_TABLE_SELECTION_SHOW         : return( ID_IMG_TB_TABLE_SELECTION_ONLY );

	case ID_CMD_HISTOGRAM_PARAMETERS         : return( ID_IMG_TB_HISTOGRAM_PARAMETERS );
	...
	case ID_CMD_DIAGRAM_SIZE_FIT             : return( ID_IMG_TB_ZOOM_FULL );
	case ID_CMD_DIAGRAM_SIZE_INC             : return( ID_IMG_TB_ZOOM_IN );
	case ID_CMD_DIAGRAM_SIZE_DEC             : return( ID_IMG_TB_ZOOM_OUT );
	}

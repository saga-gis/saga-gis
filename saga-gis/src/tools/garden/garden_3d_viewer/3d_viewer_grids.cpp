...
private:
	...
	bool						Get_Node				(CSG_Grid *pGrid, int x, int y, double z, TSG_Triangle_Node &Node);

	void						Draw_Grid				(CSG_Grid *pGrid, double z);
	...
	: CSG_3DView_Panel(pParent)
{
	...
	m_Parameters("Z_SCALE")->Set_Value(SG_Get_Length(m_pGrids->Get_XRange(), m_pGrids->Get_YRange()) / m_pGrids->Get_ZRange());
}
...
bool C3D_Viewer_Grids_Panel::On_Draw(void)
{
	...
	{
		Draw_Grid(m_pGrids->Get_Grid_Ptr(i), m_pGrids->Get_Z(i));
	}
...
//---------------------------------------------------------
inline bool C3D_Viewer_Grids_Panel::Get_Node(CSG_Grid *pGrid, int x, int y, double z, TSG_Triangle_Node &Node)
{
...
//---------------------------------------------------------
void C3D_Viewer_Grids_Panel::Draw_Grid(CSG_Grid *pGrid, double z)
{
	//-----------------------------------------------------
	...
	for(int y=1; y<pGrid->Get_NY(); y++)
	{
		for(int x=1; x<pGrid->Get_NX(); x++)
		{
			...
			if( Get_Node(pGrid, x - 1, y - 1, z, p[0])
			&&  Get_Node(pGrid, x    , y    , z, p[1]) )
			{
				if( Get_Node(pGrid, x    , y - 1, z, p[2]) )
				{
					if( Shading ) Draw_Triangle(p, false, Shade_Dec, Shade_Azi); else Draw_Triangle(p, false);
				}

				if( Get_Node(pGrid, x - 1, y    , z, p[2]) )
				{
					if( Shading ) Draw_Triangle(p, false, Shade_Dec, Shade_Azi); else Draw_Triangle(p, false);
				}
			}
...
public:
	C3D_Viewer_Grids_Dialog(CSG_Grids *pGrids)
		: CSG_3DView_Dialog(_TL("Grid Collection Viewer"))
	{
		Create(new C3D_Viewer_Grids_Panel(this, pGrids));
	}
...
//---------------------------------------------------------
void C3D_Viewer_Grids_Dialog::Set_Menu(wxMenu &Menu)
{
...
//---------------------------------------------------------
void C3D_Viewer_Grids_Dialog::On_Menu(wxCommandEvent &event)
{
	switch( event.GetId() )
	{

/**********************************************************
 * Version $Id: gdal_import_aster.cpp 1921 2014-01-09 10:24:11Z oconrad $
 *********************************************************/
...
bool CGDAL_Import_ASTER::On_Execute(void)
{
	...
	else if( Parameters("METADATA")->asTable() )
	{
		...
		pTable->Destroy();
		pTable->Set_Name(CSG_String::Format("%s [%s]", FileName.c_str(), _TL("Metadata")));
	...
	else
	{
		...
		pVNIR->Set_Name(CSG_String::Format("%s %s", FileName.c_str(), SG_T("VNIR")));
		...
		pSWIR->Set_Name(CSG_String::Format("%s %s", FileName.c_str(), SG_T("SWIR")));
		...
		pTIR ->Set_Name(CSG_String::Format("%s %s", FileName.c_str(), SG_T( "TIR")));

...
bool CShapes2Grid::On_Execute(void)
{
	...
	m_pGrid->Set_Name("%s [%s]", pShapes->Get_Name(), Field < 0 ? _TL("ID") : pShapes->Get_Field_Name(Field));
	...
	m_pCount->Set_Name("%s [%s]", pShapes->Get_Name(), _TL("Count"));
...
bool CPolygons2Grid::On_Execute(void)
{
	...
	m_pGrid->Set_Name("%s [%s]", pPolygons->Get_Name(), Field < 0 ? _TL("ID") : pPolygons->Get_Field_Name(Field));
	m_pGrid->Assign_NoData();
	...
	m_pCoverage->Set_Name("%s [%s]", pPolygons->Get_Name(), _TL("Coverage"));

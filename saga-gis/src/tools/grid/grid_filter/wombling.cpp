...
bool CWombling_Base::Get_Gradient(CSG_Grid Gradient[2], CSG_Grid *pFeature, bool bOrientation)
{
	...
	Gradient[0].Set_Name("%s [%s]", pFeature->Get_Name(), _TL("Magnitude"));
	Gradient[1].Set_Name("%s [%s]", pFeature->Get_Name(), _TL("Direction"));
...
bool CWombling::On_Execute(void)
{
	...
	Edges.Set_Name("%s [%s]", pFeature->Get_Name(), _TL("Edges"));
...
bool CWombling_MultiFeature::On_Execute(void)
{
	...
	{
		Edges.Set_Name("%s [%s]", pFeatures->Get_Grid(i)->Get_Name(), _TL("Edges"));

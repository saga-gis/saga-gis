...
CKriging_Universal::CKriging_Universal(void)
{
	...
	Set_Description(
		_TL("Universal Kriging for grid interpolation from irregular sample points.")
	);

	//-----------------------------------------------------
	pNode_0	= Parameters.Add_Shapes(
		NULL	, "SOURCE"		, _TL("Points"),
		"",
		PARAMETER_INPUT
	);
...
//---------------------------------------------------------
CShapes * CKriging_Universal::Get_Point_Shapes(CShapes *pShapes)
{
	...
	{
		pPoints	= API_Create_Shapes(SHAPE_TYPE_Point, "", &pShapes->Get_Table());
...
//---------------------------------------------------------
bool CKriging_Universal::On_Execute(void)
{
	int			x, y, n;
	CShapes		*pShapes, *pShapes_Tmp;

	//-----------------------------------------------------
	pShapes			= Parameters("SOURCE")		->asShapes();
	...
	//-----------------------------------------------------
	if( pShapes->Get_Table().Get_Field_Count() > 0 && nGrids > 0 )
	{
...
//---------------------------------------------------------
bool CKriging_Universal::Get_Grid_Value(int x, int y)
{
	int			i, j, nPoints;
...
//---------------------------------------------------------
double CKriging_Universal::Get_Weight(double d)
{
	if( d == 0.0 )
	{
		d	= 0.0001;
...
{
	int			i, j, n, iGrid, Interpolation = 1;
	double		dx, dy;
	TGEO_Point	Point;
	CShape		*pPoint;

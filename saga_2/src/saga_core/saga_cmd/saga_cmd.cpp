...
void		Print_Help		(void)
{
	SG_PRINTF(
		...
		SG_T("\n")
		SG_T("example:\n")
		SG_T("  saga_cmd Terrain_Analysis_Morphometry \n")
...
void		Create_Example	(void)
{
	...
	if( Stream.Open(FileName, SG_FILE_W, false) )
	{
		Stream.Printf(
			SG_T("@echo off\n")
			SG_T("\n")
			SG_T("set SAGA_MLB=D:\\DEVELOP\\SAGA_2.0\\bin\\saga_vc\\modules\n")
			SG_T("path=D:\\DEVELOP\\SAGA_2.0\\bin\\saga_vc;D:\\DEVELOP\\SAGA_2.0\\bin\\saga_vc\\modules\n")
			SG_T("\n")
			SG_T("echo _____________________________________________\n")
			SG_T("echo SAGASAGASAGASAGASAGASAGASAGASAGASAGASAGASAGAS\n")
			SG_T("echo some simple examples\n")
			SG_T("echo ;-)\n")
			SG_T("\n")
			SG_T("saga_cmd.exe Grid_Shapes                  0 -INPUT .\\dem.sgrd -CONTOUR .\\contour.shp -ZSTEP 25.0\n")
			SG_T("saga_cmd.exe Terrain_Analysis_Morphometry 3 -ELEVATION .\\dem.sgrd -SLOPE .\\slope.sgrd -ASPECT .\\aspect.sgrd -HCURV .\\hcurv.sgrd -VCURV .\\vcurv.sgrd\n")
			SG_T("saga_cmd.exe Terrain_Analysis_Flow        0 -ELEVATION .\\dem.sgrd -CAREA .\\carea.sgrd\n")
			SG_T("saga_cmd.exe Terrain_Analysis_Indices     1 -SLOPE .\\slope.sgrd -AREA .\\carea.sgrd -WETNESS .\\wetness.sgrd -STREAMPOW .\\streampow.sgrd -LSFACTOR .\\lsfactor.sgrd\n")
			SG_T("saga_cmd.exe Terrain_Analysis_Lighting    0 -ELEVATION .\\dem.sgrd -SHADE .\\shade.sgrd -METHOD 0 -AZIMUTH -45 -DECLINATION 45\n")
			SG_T("\n")
			SG_T("echo _____________________________________________\n")
			SG_T("echo SAGASAGASAGASAGASAGASAGASAGASAGASAGASAGASAGAS\n")
			SG_T("echo using data object lists with semicolon (;)\n")
			SG_T("echo separated file names\n")
			SG_T("echo ;-)\n")
			SG_T("\n")
			SG_T("saga_cmd.exe Grid_Discretisation          0 -INPUT .\\dem.sgrd;.\\slope.sgrd;.\\aspect.sgrd;.\\hcurv.sgrd;.\\vcurv.sgrd -RESULT .\\cluster.sgrd -STATISTICS cluster.txt -NORMALISE -NCLUSTER 10\n")
			SG_T("\n")
			SG_T("echo _____________________________________________\n")
			SG_T("echo SAGASAGASAGASAGASAGASAGASAGASAGASAGASAGASAGAS\n")
			SG_T("echo project from geodetic to gauss-krueger 3\n")
			SG_T("echo ;-)\n")
			SG_T("\n")
			SG_T("saga_cmd.exe Projection_Proj4             0 -PROJ_TYPE 98 -SOURCE .\\latlon.shp -TARGET .\\gk3.shp    -LON_0 9 -X_0 3500000\n")
			SG_T("\n")
			SG_T("echo _____________________________________________\n")
			SG_T("echo SAGASAGASAGASAGASAGASAGASAGASAGASAGASAGASAGAS\n")
			SG_T("echo using extra parameter lists\n")
			SG_T("echo (-LIST_NAME \"-OPTION1 value1 -OPTION1 value2\")\n")
			SG_T("echo ;-)\n")
			SG_T("\n")
			SG_T("saga_cmd.exe Projection_Proj4             0 -PROJ_TYPE 0  -SOURCE .\\latlon.shp -TARGET .\\albers.shp -LON_0 9 -aea \"-lat_1 25 -lat_2 40\"\n")
			SG_T("\n")
			SG_T("pause\n")
		);
